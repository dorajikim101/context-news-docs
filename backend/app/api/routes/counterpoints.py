...
def list_counterpoints(db: Session = Depends(get_db)):
    return db.query(Counterpoint).order_by(Counterpoint.created_at.desc()).all()


@router.post("", response_model=CounterpointRead)
def create_counterpoint(payload: CounterpointCreate, db: Session = Depends(get_db)):
...
def update_counterpoint(counterpoint_id: UUID, payload: CounterpointUpdate, db: Session = Depends(get_db)):
