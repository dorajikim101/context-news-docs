...
def list_action_signals(db: Session = Depends(get_db)):
    return db.query(ActionSignal).order_by(ActionSignal.created_at.desc()).all()


@router.post("", response_model=ActionSignalRead)
def create_action_signal(payload: ActionSignalCreate, db: Session = Depends(get_db)):
...
def update_action_signal(signal_id: UUID, payload: ActionSignalUpdate, db: Session = Depends(get_db)):
